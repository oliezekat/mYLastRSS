		...
		if (strtoupper($this->cp) == 'UTF-8')
			...
			if(function_exists('mb_convert_encoding'))
				...
				if (in_array(strtolower($strCP),array('auto','utf-8')))
					...
					$result=str_replace('eÃÅ','&eacute;',$result); // È
					...
					$result=str_replace('ƒç','c',$result); // c avec diacritic
					$result=str_replace('ƒÅ','a',$result); // a avec diacritic
					...
					$result=str_replace(' ‚Äç',' ',$result); //espace suivie liant sans chasse
					$result=str_replace(' ‚Å†',' ',$result); //espace fine ?
					}
